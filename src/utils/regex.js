function checkUzPhoneInt(input) {
	...
	return {
		valid: false,
		reason:
			"Raqam formati noto‘g‘ri. Kutilgan: 9 xonali mahalliy yoki 12 xonali 998 bilan boshlanadigan.",
	};
}
...
function checkTurkeyPhoneInt(input) {
	...
	return {
		valid: false,
		reason:
			"Raqam formati noto‘g‘ri. Kutilgan: 10 xonali mahalliy (5XXYYYYYYY) yoki 12 xonali xalqaro (90XXXXXXXXXX).",
	};
}
...
function checkTelegramUsername(username) {
	...
	if (!regex.test(raw)) {
		return {
			valid: false,
			reason:
				'Noto‘g‘ri format. Telegram username faqat harflar, raqamlar va "_" belgisidan iborat, uzunligi 5–32.',
		};
	}
...
function checkUsername(username) {
	...
	if (!regex.test(username)) {
		return {
			valid: false,
			reason:
				"Username faqat kichik harflar va raqamdan iborat bo‘lishi kerak, uzunligi 4–10.",
		};
	}
